...
proc ::pd_connect::to_pd {port {host localhost}} {
    variable pd_socket
    ::pdwindow::logpost 5 "'pd-gui' connecting to 'pd' on localhost $port ...\n"
...
proc ::pd_connect::from_pd {channel clientaddr clientport} {
    ...
    ::pdwindow::logpost 5 "Connection from 'pd' to 'pd-gui' on $clientaddr:$clientport\n"
    ::pd_connect::configure_socket $pd_socket
}
    ...
    if {[catch {uplevel #0 $cmd_from_pd} errorname]} {
        ...
        switch -regexp -- $errorname { 
            "missing close-brace" {
                ...
                ::pdwindow::logpost 0 [concat [_ "(Tcl) INVALID COMMAND NAME: "] $errorInfo "\n"]
            } default {
                ::pdwindow::logpost 0 [concat [_ "(Tcl) UNHANDLED ERROR: "] $errorInfo "\n"]
            }
        }
    }
}
